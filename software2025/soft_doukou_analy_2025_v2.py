...
def _(mo):
    mo.md(r"""
    mo.md(データを全区分、区分ごとに選択できるようにしたバージョン。データクレンジング処理も込)
    """)
    return
...
@app.cell
def _(df):
    #指定したカラムのNaNをvalueに置き換える
    def complement_na_exact(target_cols, value):
        #NaNを指定値に置き換える
        df[target_cols] = df[target_cols].fillna(value)
        #print(df[target_cols])
        return

    #指定したQX-YのNaNレコードを指定した値で置き換える
    #先頭がQX-Yのカラムのレコードについて処理する。
    #使用例：complement_na("Q4-1",5)
    def complement_na(regex_str, value):
        # 'abc' で始まるカラムを抽出
        regex = "^" + regex_str +"\."
        target_cols = df.filter(regex=regex).columns.tolist()
        for t in target_cols:
            complement_na_exact(t, value)
            #print(df[t])
        return

    #複数選択肢タイプの設問の値を補正する
    #すでに回答済みの箇所はすべて1(チェック済み)に設定。
    #NaNは0(未チェック)に設定する。
    #なんど呼ばれても良いように、冪等性を意識するコードとする。
    def complement_multiselection_type_na(regex_str):
        # 'abc' で始まるカラムを抽出
        regex = "^" + regex_str +"\."
        target_cols = df.filter(regex=regex).columns.tolist()
        for t in target_cols:
            # NaNを0で埋めてから、0以外の値を1にする
            #以下のように冪等性を意識
            #1回目の実行:NaN → 0 になる。
            #0以外（5や100、"A"など） → 1 になる。結果、列の中身は 「0 か 1」だけ になります。
            #2回目の実行:0 の場所 → fillna(0) で 0 のまま。
            #apply で x != 0 が False なので 0 のまま。
            #1 の場所 → fillna(0) で 1 のまま。apply で x != 0 が True なので 1 のまま。
            #print("before")
            #print(df[t])
            df[t] = df[t].fillna(0).apply(lambda x: 1 if x != 0 else 0)
            #print("after")
            #print(df[t])
    ...
    #欠損値を補完する
    def complement_na_all():
        run = True
        if run == False:
            print("SKIP：欠損値補完を★SKIP★しました！")
            return 
        NOT_TARGET = 999
        """
        [Q4-1]
        ★　ここはNOT_TARGETでうめずに、ユーザ企業だけで絞って解析するなどしたほうがいいかもしれない。
        <<Q1-4で1.ユーザー企業を選択した方のみ>>
        [質問]
        各工程における内製化の状況について、それぞれ最も当てはまるものを1つお選びください。
        1.システム企画・要求定義
        2.要件定義
        3.設計・開発・テスト
        4.運用・保守
        [回答]
        1.ほぼ全てのプロジェクトで内製化に取り組んでいる
        2.一部プロジェクトで内製化に取り組んでいる
        3.内製化しておらず外部に委託している
        4.開発を実施する必要がない
        5.わからない
        →　これについてQ1-4の1~4まででNaNを999に倒す(非該当として新たにコードを設定する)。
        """
        complement_na("Q4-1",NOT_TARGET)
        """
        [Q4-2]
        ★　ここはNOT_TARGETでうめずに、ユーザ企業だけで絞って解析するなどしたほうがいいかもしれない。
        [質問]
        <<Q1-4で1.ユーザー企業を選択した方のみ>>
        内製化に関する課題について、当てはまるものを全てお選びください。
        [回答]
        1.内製化には利点も欠点もあるため、判断が難しい
        2.人材の確保や育成が難しい
        3.新しい技術への対応が難しい
        4.開発量の増減への対応が難しい
        5.開発設備の導入・維持にコストがかかる
        6.外部発注と比較して、コスト意識が低くなる
        7.外部開発したシステムの内製化への移行が難しい
        8.わからない
        9.その他（自由記述）
        10.特になし／内製化していない
        →　これについてQ1-4の1~4まででNaNを999に倒す(非該当として新たにコードを設定する)。
        """
        complement_na("Q4-2",NOT_TARGET)
        """
        [Q5-3]
        [質問]
        レガシーシステム刷新の課題について、当てはまるものを全てお選びください。
        [回答]
        1.経営者に必要性が理解されない
        2.経営者に移行の困難さが理解されず予算や納期を確保できない
        3.ユーザの既存システムの操作性へのこだわりを解消できない
        4.ブラックボックス化によりレガシーシステムの解析が困難
        5.レガシーシステムが肥大化し移行の影響度が想定できない
        6.古い技術を理解した技術者の確保ができない
        7.他の案件に手一杯で十分な要員を割けない
        8.レガシーシステム刷新に長けたプロジェクトリーダーがいない
        9.その他（自由記述）
        10.特になし
        →　これについて、NaNは10で埋める。
        """
        complement_multiselection_type_na("Q5-3")
        """
        [Q6-2]
        [質問]
        貴社におけるベンダーロックインの主な原因について、当てはまるものを全てお選びください。
        [回答]
        1.特定のベンダーだけが持つ独自規格や独自技術がシステムに組み込まれている
        2.システム間が密結合しており、全体構成が複雑・肥大化しているため、新規ベンダーが参入しづらい
        3.設計情報や技術仕様が不足している
        4.システムに関する知識・ノウハウを持つ社内の有識者がいない
        5.システムや技術の知的財産がベンダーに帰属している
        6.契約上、ソースコードや設計書の共有・利用に制約がある
        7.ベンダーと長期契約を結んでおり、移行時に違約金などのコストが発生する
        8.わからない
        9.その他（自由記述）
        10.特になし／ベンダーロックインはない
        """
        complement_multiselection_type_na("Q6-2")
        """
        [Q6-3]
        [質問]
        貴社において、ベンダーロックインを回避・軽減するために実施している取り組みについて、当てはまるものを全てお選びください。
        [回答]
        1.システムの疎結合化やモジュール化を意識して設計・構築している
        2.オープン標準やデファクトスタンダードな技術・製品を採用している
        3.仕様や設計情報の可視化を行い、ブラックボックス化を防いでいる
        4.知的財産権やソースコードの帰属を契約書に明記している
        5.わからない
        6.その他（自由記述）
        7.特になし／ベンダーロックインはない
        """
        complement_multiselection_type_na("Q6-3")
        """
        [Q9-2]
        [質問]
        貴社が取り組んでいるオープンソースプロジェクトへの貢献活動について、当てはまるものを全てお選びください。
        [回答]
        1.社員によるソースコードの提供
        2.保守契約や商用サポートの購入によるOSS維持のための間接的な支援
        3.コミュニティのフォーラムやイベント等での知見共有や議論参加
        4.プロジェクトやイベントへのスポンサーシップ等による金銭的な支援
        5.社内開発したツールやライブラリ等をオープンソース化し、再利用を促進
        6.わからない
        7.その他（自由記述）
        8.特に活動していない
        """
        complement_multiselection_type_na("Q9-2")
        """
        [Q9-4]
        [質問]
        貴社におけるOSS活用に関する課題について、当てはまるものをすべてお選びください。
        [回答]
        1.OSSの利用や公開に関する社内ルール・ポリシーが存在しない
        2.OSSのライセンス遵守や知的財産に関する社内体制が整っていない
        3.セキュリティ面の懸念がある（脆弱性対応や更新管理など）
        4.商用サポートがなく、安心して使いづらい
        5.OSSに関する技術的なノウハウや人材が社内に不足している
        6.OSS利用を推奨する社内文化が育っていない
        7.利用部門や発注元がOSSの利用に消極的である
        8.経営層の理解・支援が不足している
        9.信頼できる公的・中立的なOSS情報サイトや支援窓口がない
        10.わからない
        11.その他（自由記述）
        12.特になし／OSSを利用していない
        """
        complement_multiselection_type_na("Q9-4")
        """
        [Q10-3]
        [質問]モデリングツールの課題や利用できない理由について、当てはまるものを全てお選びください。
        [回答]
        1.改修開発の前提となる既存のソフトウェアがモデリングを使っていない
        2.モデリングができる技術者が不足している
        3.契約先の会社がモデリングを提案してこない
        4.モデリングツールの機能が十分ではない
        5.従来のドキュメント記述で対応できるのでモデリングは必要ない
        6.モデリングで書くことを利用部門や要求元が望んでいない
        7.モデリングツールのライセンス費用などにコストがかかる
        8.モデリングツールのメリットがわからない
        9.わからない
        10.その他（自由記述）
        11.特になし／開発を実施していない
        """
        complement_multiselection_type_na("Q10-3")
        #※以下テンプレ
        """
        [QX-Y]
        [質問]
...
def _(df, plt, sangyou_kubun_mapping):
    def show_ea():
        col = "Q13-1.その他テクノロジーの導入状況(7.エンタープライズアーキテクチャー（EA）)"
    
        # 1. データの集計
        #欠損処理後に注意、1か0のみ格納
        df_temp = df[df[col] == 1] #EAを使っているという組織を集計
        print(f"EAを使っている組織数={len(df_temp)}")
        print(f"全体の組織数={len(df)}")
        print(f"割合={len(df_temp)/len(df)*100} %")
        counts = df_temp['Q1-3.産業区分'].map(sangyou_kubun_mapping).value_counts()
